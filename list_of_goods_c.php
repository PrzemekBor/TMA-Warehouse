...
<div id="order">
    <div class="box1">
        <a id="button" href="#popup1">ADD</a>
        <a id="button" href="#popup2">UPDATE</a>
        <a id="button" href="#popup3">REMOVE</a>
    </div>
</div>
    <table>
        <tr>
    ...
    <div id="popup1" class="overlay">
        <div class="popup">
            ...
            <form action="add.php" method="post">
                <input type="text" name="item_group" required><label>Item Name</label>
                <input type="text" name="unit_of_measurement" required><label>Unit</label>
                <input type="text" name="quantity" required><label>Quantity</label>
                <input type="text" name="price_UAH" required><label>Price</label>
                <input type="text" name="status" required><label>Status</label>
                <input type="text" name="storage_location" required><label>Storage</label>
                <textarea id="textarea" name="contact_person" rows="4" cols="50"></textarea>
                <label for="textarea">Contact Person</label>
                <input type="file" name="photo"><label>Image</label>
    ...
    <div id="popup2" class="overlay">
        <div class="popup">
            ...
            <form action="update.php" method="post">
                ...
                ?>
                <select name="item_group" id="choice"><option>Item name</option>
                    <?php
                ...
                <select name="unit_of_measurement" id="choice"><option>Unit of measurement</option>
                    <?php
                    foreach ($options as $option) {
                        ?>
                ...
                <input type="text" name="quantity"><label>Quantity</label>
                <input type="text" name="price_UAH"><label>Price UAH</label>
                <input type="text" name="status"><label>Status</label>
                <input type="text" name="storage_location"><label>Storage location</label>
                <textarea id="textarea" name="contact_person" rows="4" cols="50"></textarea>
                <label for="textarea">Contact Person</label>
                <input type="hidden" name="send" value="send"/>
                <input type="submit" value="SEND">
...
<div id="popup3" class="overlay">
    <div class="popup">
        <h2>Select the item you want to remove:</h2>
        <a class="close" href="#">&times;</a>
        <form action="drop.php" method="post">
            <?php
            $query ="SELECT item_group FROM items";
            $result = $data->query($query);
            if($result->num_rows> 0){
                $options= mysqli_fetch_all($result, MYSQLI_ASSOC);
            }
            ?>
            <select name="item_group" id="choice"><option>Select item</option>
                <?php
                foreach ($options as $option) {
                    ?>
                    <option><?php echo $option['item_group']; ?> </option>
