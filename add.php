...
if(isset($_POST) && !empty($_POST['send']) && $_POST['send']=='send') {
    ...
    $photo = $to_db_data['photo'];

    if ($db->num_rows($db->query("SELECT * FROM items WHERE item_id=$item_group ;")) == 0) {
        if ($db->query("INSERT INTO items (`item_group`,`unit_of_measurement`, `quantity`, `price_UAH`, `status`, `storage_location`, `contact_person`, `photo`)
    values ($item_group, $unit_of_measurement, $quantity, $price_UAH,$status,$storage_location,$contact_person,$photo)")) {
